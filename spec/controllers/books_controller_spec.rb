...
shared_examples "get_index" do
  it "populates an array of books order by desc id" do
    books = []
  ...
  it "populates an array of books order by desc id with certain category" do
...
shared_examples "put_update" do
  it "assigns the requested book to @book" do
    put :update, id: @book.id, book: @book_attr || @invalid_book_attr
    expect(assigns :book).to eq @book
  end

  it "redirects to index if current does not own the book" do
    put :update, id: FactoryGirl.create(:book).id, book: @book_attr || @invalid_book_attr
...
describe BooksController do
  ...
  context "SIGNED IN" do
    ...
    describe "GET #new" do
      ...
      it "builds a new book that belongs current user" do
    ...
    describe "GET #edit" do
      ...
      it "assigns the requested book to @book with user eql current user" do
        expect(assigns(:book).user).to eq @book.user
      end

      it "redirects to index if current user does not own the requested book" do
        get :edit, id: FactoryGirl.create(:book)
        expect(response).to redirect_to :books
      end

      it "renders the edit view if current user owns the requested book" do
    ...
    describe "POST #create" do
      context "with valid attributes" do
        before do
          @book_attr = FactoryGirl.attributes_for(:book)
                       .merge(category_id: FactoryGirl.create(:category).id)
        end

        it "creates a new book that belongs to current user" do
          expect do
      ...
      context "with invalid attributes" do
        before do
          @invalid_book_attr = FactoryGirl.attributes_for(:invalid_book)
                               .merge(category_id: FactoryGirl.create(:category).id)
        end

        it "does not create a new book" do
          expect do
            post :create, book: @invalid_book_attr
    ...
    describe "PUT #update" do
      context "with valid attributes" do
        ...
        it "redirects to the show view of updated book" do
          put :update, id: @book.id, book: @book_attr
      ...
      context "with invalid attributes" do
        ...
        it "does not change book attributes" do
          put :update, id: @book.id, book: @invalid_book_attr
          @book.reload
          expect(@book.title).to_not eq @invalid_book_attr[:title]
          expect(@book.description).to_not eq @invalid_book_attr[:description]
          expect(@book.author).to_not eq @invalid_book_attr[:author]
          expect(@book.category_id).to_not eq @invalid_book_attr[:category_id]
        end
    ...
    describe "DELETE #destroy" do
      ...
      it "delete book from current user" do
        expect do
          delete :destroy, id: @book.id
        end.to change(@user.books, :count).by(-1)
      end
